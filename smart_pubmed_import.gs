...
// ==========================================
// WRAPPER: importPapersFromPubMed()
// ==========================================
// Wird von ui_pubmed_menu_extended.gs aufgerufen:
//   importPapersFromPubMed(query, startDate, maxResults, category)
//
// Bridgt zwischen dem alten Menu-System und
// der eigentlichen Import-Logik in pubmed_import.gs
// ==========================================

/**
 * ✅ Wrapper für ui_pubmed_menu_extended.gs
 * Wird von showPredefinedPubMedSearches(), quickImport*() aufgerufen
 *
 * @param {string} query       - PubMed Suchquery
 * @param {string} startDate   - Startdatum (YYYY/MM/DD) oder null
 * @param {number} maxResults  - Max. Anzahl Papers
 * @param {string} category    - Kategorie-Label (nur für Logging, z.B. ""Diabetes"")
 */
function importPapersFromPubMed(query, startDate, maxResults, category) {
  var ui = SpreadsheetApp.getUi();

  Logger.log('importPapersFromPubMed gestartet');
  Logger.log('  Query:    ' + query);
  Logger.log('  Datum:    ' + (startDate || 'neueste'));
  Logger.log('  Max:      ' + maxResults);
  Logger.log('  Kategorie:' + (category || '-'));
  ...
  try {
    // Nutzt die Funktion aus pubmed_import.gs
    // (importFromPubMedWithDate ist dort definiert)
    importFromPubMedWithDate(query, startDate, maxResults);

    // Kategorie im Log festhalten
    if (typeof logAction === 'function') {
      logAction('PubMed Import', 'Kategorie: ' + (category || 'Unbekannt') + ' | Query: ' + query);
    }
